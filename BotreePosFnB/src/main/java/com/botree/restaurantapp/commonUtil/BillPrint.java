...
class BillPrint implements Printable {
	...
	public int print(Graphics graphics, PageFormat pageFormat, int pageIndex)
      ...
      /*Extra Line For Suufiyana*/
      if (storeId == 161) {
        String extraLine = "A UNIT OF SUFEEN CUISINE PVT.LTD.";
        String extraLine2 = WordUtils.wrap(extraLine, 40);
        String split[] = extraLine2.split("\\n");
        for (int k = 0; k < split.length; k++) {
          g2d.drawString(split[k], 0, y);
			...
			for (int i = 1; i < mod.length; i++) { // items
				...
				if (i != (mod.length - 1)) {
					...
					if (length1 <= 16) {
						...
						g2dItem.drawString(quantity, 95, cH);
						...
						//g2dItem.drawString(quantity, 105, cH);
						g2dItem.drawString(quantity, 95, cH);//for balaram
